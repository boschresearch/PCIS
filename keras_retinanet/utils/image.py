...
def preprocess_image(x,
                     mode='caffe'):
    """ 
    ImageNet weights for VGG AND RESNET WERE PRETRAINED USING CAFFE; 
    Weights for MobileNet use Tensorflow tf; see
...
def random_visual_effect_generator(
    ...
    if contrast_range:
        ...
        uniform_contrast  = _uniform(contrast_range)
    else:
        uniform_contrast = None
    if brightness_range:
        _check_range(brightness_range, -1, 1)
        uniform_brightness = _uniform(brightness_range)
    else:
        uniform_brightness = None
    if hue_range:
        _check_range(hue_range, -1, 1)
        uniform_hue = _uniform(hue_range)
    else:
        uniform_hue = None
    if saturation_range:
        _check_range(saturation_range, 0)
        uniform_saturation = _uniform(saturation_range)
    else:
        uniform_saturation = None

    def _generate():
        while True:
            yield VisualEffect(
                contrast_factor=uniform_contrast,
                brightness_delta=uniform_brightness,
                hue_delta=uniform_hue,
                saturation_factor=uniform_saturation
            )
    return _generate()
